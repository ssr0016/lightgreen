...
func (m MovieModel) Get(id int64) (*Movie, error) {
	...
	// 3-second timeout deadline, Use defer to make sure that we cancel the context before the Get()
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
func (m MovieModel) GetAll(title string, genres []string, filters Filters) ([]*Movie, Metadata, error) {
	...
	// passing in the total records and pagination
...
func ValidateMovie(v *validator.Validator, movie *Movie) {
	...
	v.Check(validator.Unique(movie.Genres), "genres", "must not containduplicate values")
}
